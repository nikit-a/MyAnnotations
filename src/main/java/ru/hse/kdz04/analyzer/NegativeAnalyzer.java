...
public class NegativeAnalyzer implements Analyzer {
    @Override
        ...
        if (obj instanceof Byte || obj instanceof Short || obj instanceof Integer || obj instanceof Long) {
            ...
            if (num.longValue() >= 0) {
                return Optional.of(new ValidationErrorClass("У числа " + num +
                        " не отрицательное значение", path, obj));
            }
        } else {
            throw new IncorrectAnnotatedTypeException("Объект не является типом " +
                    "byte, short, int, long и их обёртками к которым применяется аннотация Negative");
        }
        return Optional.empty();
    }
}
